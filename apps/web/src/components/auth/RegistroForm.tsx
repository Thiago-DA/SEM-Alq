 ...
 * Diseño: Claude Design, "Autenticación" · 03 (paso 2) y 05 (estados).
 *
 * NOTA: el paso 1 todavía no tiene diseño en Claude Design (el PO lo está
 * agregando). Mientras tanto el formulario arranca en el paso 2, con el rol
 * que llega por la URL (`?rol=locador`) o deducido del `next` (si venía a
 * publicar una propiedad, es locador); si no, locatario. "Atrás" vuelve al
 * login. Cuando esté el diseño, basta con poner `STEP_ROL_READY = true` y
 * dibujar el paso 1 en `renderStepRol`.
 *
...
/**
 * `false` mientras el paso 1 ("¿Alquilás o publicás?") no tenga diseño.
 * Ver la NOTA del encabezado.
 */
const STEP_ROL_READY = false
...
interface RegistroFormProps {
  /** Rol con el que arranca el formulario (de `?rol=` o deducido de `next`). */
  initialRol: RolRegistro
...
export function RegistroForm({ initialRol, next }: RegistroFormProps) {
  ...
  // ─── Estado local ───────────────────────────────────────────────────
  // El rol vive en el estado del formulario (no en la URL): el paso 1 lo va
  // a cambiar y el paso 2 lo manda al service.
  const [rol] = useState<RolRegistro>(initialRol)
  const [step, setStep] = useState<Step>(STEP_ROL_READY ? 'rol' : 'datos')
  ...
  /** "Atrás": al paso 1 si existe; mientras no tenga diseño, al login. */
  ...
  /**
   * Paso 1: "¿Alquilás o publicás?".
   * TODO: dibujarlo cuando esté su vista en Claude Design (ver la NOTA del
   * encabezado). No se muestra mientras `STEP_ROL_READY` sea `false`.
   */
  function renderStepRol() {
    return null
  }
  ...
  const title = step === 'listo' ? 'Cuenta creada' : step === 'rol' ? 'Crear cuenta' : 'Tus datos'

  return (
    <AuthLayout title={title} data-testid="registro-page">
      {step === 'rol' && renderStepRol()}
